...
function DebateContent() {
  ...
  useEffect(() => {
    ...
    if (philosopherIdsFromUrl.length > 0) {
      ...
      const loadWithPhilosophers = (phils: Philosopher[]) => {
        ...
        const doLoadConversation = (convId: string, allPhilosopherIds: string[]) => {
          loadConversationMessages(convId, allPhilosopherIds, phils);
        };

        if (existingConvId) {
          // First get the full conversation to get all philosopher IDs
          fetch(`/api/conversations?userId=null&conversationId=${existingConvId}`)
            .then(res => res.ok ? res.json() : null)
            .then((convData: { philosopherIds?: string[]; id?: string } | null) => {
          ...
          fetch(`/api/conversations?userId=null`)
            ...
            .then((allConvs: { philosopherIds?: string[]; id?: string }[]) => {
              ...
              const matchingConv = allConvs.find((c) => { 
                ...
                return convPhilosopherIds && 
                  philosopherIdsFromUrl.every((pid: string) => convPhilosopherIds.includes(pid));
              });
              if (matchingConv && matchingConv.id) {
                ...
                doLoadConversation(existingConvId, matchingConv.philosopherIds || philosopherIdsFromUrl);
              }
            })
            .catch(e => console.error("Failed to fetch conversation from API:", e));
        }
      };
  ...
  const loadConversationMessages = async (
    ...
    try {
      ...
      const convRes = await fetch(`/api/conversations?userId=null&conversationId=${convId}`);
      ...
      if (msgRes.ok) {
        ...
        existingMessages.forEach((m: { role: string; content: string; philosopherId?: string; philosopherName?: string }, idx: number) => {
          if (m.role === "user") {
            ...
            if (!philosopherId) {
              ...
              philosopherId = actualPhilosopherIds[currentRoundMsgs.length % actualPhilosopherIds.length];
            }
            const philosopherName = m.philosopherName || philList.find(p => p.id === philosopherId)?.name || "Philosopher";
  ...
  const handleSubmitQuestion = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errorResults = data.results.filter((r: { error: string | null }) => r.error);
        ...
        if (newMessages.length === 0) {
          ...
          const failMessages: ComparisonMessage[] = selectedPhilosophers.map((id, idx) => {
            const philosopher = philosophers.find((p) => p.id === id);
            const errorInfo = errorResults.find((e: { philosopherId: string }) => e.philosopherId === id);
            return {
              id: `fail-${Date.now()}-${idx}`,
              philosopherId: id,
              philosopherName: philosopher?.name || "Unknown",
              content: errorInfo ? `Error: ${errorInfo.error}` : "No response received",
              round,
              question: currentQuestion,
            };
          });
      ...
      <main className="pt-16 pb-32 min-h-screen flex flex-col">
        ...
        {showResults && (messages.length > 0 || isLoading) && (
          <section className="p-6 border-t border-outline-variant/20">
            ...
            {currentRound > 0 && (
              <div className="mt-6 p-4 bg-surface-container border border-outline-variant/10 rounded-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    onClick={() => setQuestion("What are the practical implications of your philosophical position?")}
                    className="px-3 py-2 bg-surface border border-outline-variant/20 rounded-sm text-xs text-zinc-400 hover:text-secondary hover:border-secondary/30 transition-all"
                  >
            ...
            {isLoading && (
              <div className="mt-6 bg-surface-container border border-outline-variant/20 rounded-sm p-8 text-center" aria-live="polite" aria-label="Loading">
            ...
            <div className="text-center">
              ...
              <div className="flex flex-col gap-2 items-center">
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  <button
                    onClick={() => setQuestion("What is the nature of virtue and how can we cultivate it?")}
                  ...
                  <button
                    onClick={() => setQuestion("How should we balance individual freedom with social responsibility?")}
                    className="px-3 py-2 bg-surface-container border border-outline-variant/30 rounded-sm text-xs text-zinc-400 hover:text-primary hover:border-primary/30 transition-all"
                  >
                    Freedom vs. social responsibility
                  </button>
                  <button
                    onClick={() => setQuestion("Is there an objective truth, or is everything relative to perspective?")}
                    className="px-3 py-2 bg-surface-container border border-outline-variant/30 rounded-sm text-xs text-zinc-400 hover:text-primary hover:border-primary/30 transition-all"
                  >
                    Objective truth or relative?
                  </button>
                  <button
                    onClick={() => setQuestion("What role should emotion play in moral decision-making?")}
                    className="px-3 py-2 bg-surface-container border border-outline-variant/30 rounded-sm text-xs text-zinc-400 hover:text-primary hover:border-primary/30 transition-all"
                  >
                    Emotion in moral decisions
                  </button>
                  <button
                    onClick={() => setQuestion("How can we live a meaningful life in a world without inherent purpose?")}
                    className="px-3 py-2 bg-surface-container border border-outline-variant/30 rounded-sm text-xs text-zinc-400 hover:text-primary hover:border-primary/30 transition-all"
                  >
                    Meaning in an indifferent universe
                  </button>
                  <button
                    onClick={() => setQuestion("What does it mean to truly know oneself?")}
                    className="px-3 py-2 bg-surface-container border border-outline-variant/30 rounded-sm text-xs text-zinc-400 hover:text-primary hover:border-primary/30 transition-all"
                  >
                    Knowing oneself
                  </button>

...
function ArenaContent() {
      ...
      if (philosophers.length === 0) {
        fetch("/api/philosophers")
          ...
          .then((data: Philosopher[]) => {
            const matchedPhilosophers = data.filter((p) => philosopherIds.includes(p.id));
            if (matchedPhilosophers.length > 0) {
              handleSelectPhilosopher(matchedPhilosophers, targetMode);
            }
          });
      } else {
        const matchedPhilosophers = philosophers.filter((p) => philosopherIds.includes(p.id));
        if (matchedPhilosophers.length > 0) {
          handleSelectPhilosopher(matchedPhilosophers, targetMode);
        }
      }
  ...
  const handleSelectPhilosopher = (philosophersToSelect: Philosopher | Philosopher[], forcedMode?: Mode) => {
    // Normalize input to array
    const philosopherArray = Array.isArray(philosophersToSelect) ? philosophersToSelect : [philosophersToSelect];
    const firstPhilosopher = philosopherArray[0];
    const selectedIds = philosopherArray.map(p => p.id);
    const targetMode = forcedMode || (philosopherArray.length > 1 ? "compare" : "single");
      ...
      if (existingConvId) {
        ...
        fetch("/api/conversations")
          ...
          .then(data => {
            ...
            const matchingConv = data.find((conv: { philosopherIds?: string[]; id?: string }) => 
              conv.philosopherIds && 
    ...
    // Update last used
    const updatedLastUsed = [...selectedIds, ...lastUsedIds.filter(id => !selectedIds.includes(id))].slice(0, 5);
        ...
        if (mode === "single" && philosopherIds.length === 1) {
          ...
          const mappedMessages = existingMessages
            ...
            .map((m: { role: string; content: string }, idx: number) => ({
              ...
              content: m.content,
              timestamp: m.role === "user" ? "You" : (philosopherIds[0] ? availablePhilosophers?.find(p => p.id === philosopherIds[0])?.name || "Thinker" : "Thinker"),
            }));
          setChatMessages(mappedMessages);
          const lastUserMsg = [...mappedMessages].reverse().find((m: ChatMessage) => m.role === "user");
          if (lastUserMsg) {
          ...
          let currentRoundMsgs: { philosopherId: string; philosopherName: string; content: string }[] = [];
          let currentRound = 0;
          let lastUserQuestion = "";
          
          existingMessages.forEach((m: { role: string; content: string; philosopherId?: string; philosopherName?: string }) => {
            if (m.role === "user") {
              ...
              if (!philosopherId) {
                philosopherId = philosopherIds[currentRoundMsgs.length % philosopherIds.length];
              }
              const philosopherName = m.philosopherName || availablePhilosophers?.find(p => p.id === philosopherId)?.name || "Thinker";
              
              currentRoundMsgs.push({ philosopherId, philosopherName, content: m.content });
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.conversationId) {
          ...
          const convType = selectedPhilosopherData.length > 1 ? "debate" : "dialogue";
          selectedPhilosophers.forEach(pId => {
            newHistory[pId] = { topic: question.trim().slice(0, 50), date: new Date().toISOString(), type: convType, philosopherIds: selectedPhilosopherData.map(p => p.id) };
          });
        ...
        if (mode === "single" && selectedPhilosophers.length === 1) {
          ...
          const assistantMessage: ChatMessage = {
            ...
            timestamp: selectedPhilosophers[0] ? philosophers.find(p => p.id === selectedPhilosophers[0])?.name || "Thinker" : "Thinker",
          };
          ...
          const newMessages: ComparisonMessage[] = data.results
            .filter((r: { response: string | null; error: string | null }) => r.response && !r.error)
            .map((response: { philosopherId: string; philosopherName: string; response: string }, idx: number) => ({
              id: `msg-${Date.now()}-${idx}`,
  ...
  const handleSendChatMessage = async (text: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const historyStored = localStorage.getItem("ai-phi-conversation-history");
        const newHistory = historyStored ? JSON.parse(historyStored) : {};
        newHistory[selectedPhilosopher.id] = { topic: text.trim().slice(0, 50), date: new Date().toISOString(), type: "dialogue", philosopherIds: [selectedPhilosopher.id] };
        localStorage.setItem("ai-phi-conversation-history", JSON.stringify(newHistory));
      }
    } catch (error) {
  ...
  const selectedPhilosopherData = philosophers.filter(p => selectedPhilosophers.includes(p.id));
  ...
  const content = selectedPhilosopher ? philosopherContent[selectedPhilosopher.id] : null;
        ...
        <main className="pt-16 pb-40 min-h-screen">
          ...
          <div className="relative h-48 overflow-hidden">
            {selectedPhilosopher.imageUrl ? (
              <img src={selectedPhilosopher.imageUrl} alt="" className="absolute inset-0 w-full h-full object-cover opacity-30 grayscale" />
            ) : (
            ...
            <div className="relative z-10 p-6 flex items-end gap-4 h-full">
              <button onClick={handleBack} className="mb-1 p-2 hover:bg-surface-container rounded-sm transition-colors">
          ...
          {/* Context indicator */}
          {lastDiscussedMap[selectedPhilosopher.id] && chatMessages.length === 0 && (
            <div className="px-6 py-2 border-b border-outline-variant/20 bg-surface-container/50">
              <div className="flex items-center gap-2 text-xs text-primary">
                <span className="material-symbols-outlined text-sm">auto_awesome</span>
                <span>Resuming conversation about "{lastDiscussedMap[selectedPhilosopher.id].topic}"</span>
              </div>
            </div>
          )}
      ...
      <main className="pt-16 pb-32 min-h-screen flex flex-col">
        ...
        <section className="p-6 border-b border-outline-variant/20">
            ...
            <div className="flex bg-surface-container rounded-sm p-1">
              <button
                ...
                className={`px-4 py-2 rounded-sm font-label text-[10px] uppercase tracking-widest transition-all ${
                  mode === "single" ? "bg-primary text-surface-container-lowest" : "text-zinc-400 hover:text-white"
                }`}
              >
                Single
              </button>
              <button
                onClick={() => setMode("compare")}
                className={`px-4 py-2 rounded-sm font-label text-[10px] uppercase tracking-widest transition-all ${
                  mode === "compare" ? "bg-primary text-surface-container-lowest" : "text-zinc-400 hover:text-white"
                }`}
              >
                Compare
              </button>
          ...
          {loadingPhilosophers ? (
            ...
            /* Single Mode - Grid */
            <div>
              <p className="font-label text-[10px] text-zinc-500 uppercase tracking-widest mb-3">Select a Thinker</p>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-6 lg:grid-cols-9 gap-3 px-6">
                {philosophers
                  .filter(p => {
                    if (mode === "single" || selectedPhilosophers.length === 0 || selectedPhilosophers.includes(p.id)) {
                  ...
                  .map((philosopher) => {
                    ...
                    <button
                      ...
                      className={`rounded-sm overflow-hidden group cursor-pointer transition-all border-2 focus:outline-none focus:ring-2 focus:ring-primary/30 ${
                        isSelected ? "border-primary" : "border-outline-variant/30 hover:border-primary/30"
                      }`}
                    >
                      <div className="h-64 relative">
                        ...
                        {philosopher.imageUrl ? (
                          ...
                          <div className="absolute inset-0 w-full h-full bg-surface-container flex items-center justify-center">
                            <span className="font-headline text-4xl text-zinc-600">{philosopher.name.charAt(0)}</span>
                          </div>
                        )}
            ...
            <div>
              <p className="font-label text-[10px] text-zinc-500 uppercase tracking-widest mb-3">Select Thinkers (1+ for compare)</p>
              ...
              {selectedPhilosophers.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                  <span className="font-label text-[10px] text-zinc-500 uppercase">Selected: {selectedPhilosophers.length}</span>
                  {selectedPhilosopherData.map(p => (
                    <span key={p.id} className="px-2 py-1 bg-surface-container-high text-primary text-[10px] font-label border border-primary/20">
                      {p.name}
                    </span>
                  ))}
                </div>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex gap-2">
              <textarea
                ...
                placeholder={showResults ? "Continue the conversation..." : "What question would you like to explore?"}
                rows={1}
              ...
              <button
                ...
                disabled={!question.trim() || selectedPhilosophers.length === 0 || isLoading || isLoadingMessages}
                className={`px-6 py-3 rounded-sm font-headline font-bold uppercase tracking-widest flex items-center gap-2 transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-primary/30 ${
                  question.trim() && selectedPhilosophers.length > 0 && !isLoading && !isLoadingMessages
          ...
          <section className="p-6 border-t border-outline-variant/20 flex-1">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-headline text-lg uppercase tracking-widest">
                  {selectedPhilosophers.length > 1 ? "Comparative Responses" : "Response"}
                </h2>
              </div>
              <button
                onClick={handleReset}
            ...
            {currentRound > 0 && (
              <div className="mt-6 p-4 bg-surface-container border border-outline-variant/10 rounded-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    onClick={() => setQuestion("Based on your earlier responses, how would you respond to the opposing view?")}
                  ...
                  <button
                    onClick={() => setQuestion("Can you provide an example from contemporary life that illustrates your position?")}
                    className="px-3 py-2 bg-surface border border-outline-variant/20 rounded-sm text-xs text-zinc-400 hover:text-secondary hover:border-secondary/30 transition-all"
                  >
                    Modern example?
                  </button>
                  <button
                    onClick={() => setQuestion("What would you say to someone who disagrees with your fundamental premise?")}
                    className="px-3 py-2 bg-surface border border-outline-variant/20 rounded-sm text-xs text-zinc-400 hover:text-secondary hover:border-secondary/30 transition-all"
                  >
                    Address objections?
                  </button>
          ...
          <section className="flex-1 p-6 flex items-center justify-center">
            <div className="text-center">
              ...
              <div className="flex flex-col gap-2 items-center">
                <p className="font-label text-[10px] text-zinc-500 uppercase tracking-widest">Leading Questions</p>
